...
const { getSettings, getShop, upsertSettings, upsertShop } = require('./db');
const {
  adminGraphQL,
  API_VERSION,
  buildAuditSummary,
...
['SHOPIFY_API_KEY', 'SHOPIFY_API_SECRET', 'SHOPIFY_SCOPES', 'APP_URL'].forEach(requireEnv);
...
app.post('/api/settings', ensureInstalled, (req, res) => {
  ...
  const updated = upsertSettings(payload);
  res.json(updated);
});
...
app.get('/api/scan', ensureInstalled, async (req, res) => {
  try {
    ...
    const rows = images.map((item) => {
      ...
      const score = item.fileSizeKb == null
        ? 50
        : kb < threshold * 0.5
          ? 95
          : kb < threshold
      ...
      const recommendation = item.fileSizeKb == null
        ? 'Shopify did not expose file size. Keep dimensions tight and upload compressed WebP/JPG sources.'
        : kb >= threshold * 2
          ? 'Very large image. Resize and compress aggressively.'
          : kb >= threshold
...
app.listen(PORT, () => {
  console.log(`Shopify image audit app running on http://localhost:${PORT}`);
});

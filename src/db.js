...
function listShops() {
  return db.prepare('SELECT shop, scope, installed_at, updated_at FROM shops ORDER BY updated_at DESC').all();
}

function getSettings(shop) {
  const existing = db.prepare('SELECT * FROM settings WHERE shop = ?').get(shop);

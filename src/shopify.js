...
function verifyHmac(query) {
  ...
  const generated = crypto.createHmac('sha256', process.env.SHOPIFY_API_SECRET).update(message).digest('hex');
...
async function adminGraphQL({ shop, accessToken, query, variables = {} }) {
  ...
  if (response.data.data && Object.values(response.data.data).some((v) => v?.userErrors?.length)) {
    ...
    for (const value of Object.values(response.data.data)) {
      if (value?.userErrors?.length) messages.push(...value.userErrors.map((e) => e.message));
    }
...
async function registerAppUninstalledWebhook({ shop, accessToken }) {
  ...
  return adminGraphQL({
    ...
    variables: {
      ...
      uri: `${process.env.APP_URL}/webhooks/app/uninstalled`,
    },
  });
}
...
function buildAuditSummary(images, thresholdKb = 400) {
  ...
  const averageKb = total ? Math.round(images.reduce((sum, item) => sum + (item.fileSizeKb || 0), 0) / total) : 0;
